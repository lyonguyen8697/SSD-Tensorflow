...
class SemiSSDNet(object):
    ...
    def __init__(self, params=None):
        ...
        if isinstance(params, SSDParams):
            ...
            self.params = SemiSSDNet.default_params
    ...
    def semi_losses(self, labeled_preds,
                    labeled_ground_truths,
                    unlabeled_preds,
                    unlabeled_pseudo_labels,
                    global_step,
                    step_per_epoch,
                    T1=20,
                    T2=120,
                    ...
                    alpha=1.,
                    label_smoothing=0.,
                    scope='semi_ssd_losses'):
        """Define the SSD network losses.
        """
        return semi_ssd_losses(labeled_preds,
                               labeled_ground_truths,
                               unlabeled_preds,
                               unlabeled_pseudo_labels,
                               global_step,
    ...
    def get_unlabeled_pseudo_labels(self, gclasses, glocalisations, gscores):

        with tf.variable_scope('unlabeled_pseudo_labels'):
            pclasses = [tf.get_variable('class_{}'.format(i), gclass.get_shape(), gclass.dtype, trainable=False) for i, gclass in enumerate(gclasses)]
            plocalisations = [tf.get_variable('localisation_{}'.format(i), glocalisation.get_shape(), glocalisation.dtype, trainable=False) for
                        i, glocalisation in enumerate(glocalisations)]
            pscores = [tf.get_variable('score_{}'.format(i), gscore.get_shape(), gscore.dtype, trainable=False) for
                        i, gscore in enumerate(gscores)]
        return pclasses, plocalisations, pscores
...
def ssd_net(inputs,
            num_classes=SemiSSDNet.default_params.num_classes,
            feat_layers=SemiSSDNet.default_params.feat_layers,
            anchor_sizes=SemiSSDNet.default_params.anchor_sizes,
            anchor_ratios=SemiSSDNet.default_params.anchor_ratios,
            normalizations=SemiSSDNet.default_params.normalizations,
            is_training=True,
            dropout_keep_prob=0.5,
            prediction_fn=slim.softmax,
            reuse=None,
    ...
    end_points = {}
    with tf.variable_scope(scope, 'semi_ssd_300_vgg', [inputs], reuse=reuse):
...
# =========================================================================== #
# Semi SSD loss function.
# =========================================================================== #
def semi_ssd_losses(labeled_preds,
                    labeled_ground_truths,
                    unlabeled_preds,
                    unlabeled_pseudo_labels,
                    global_step,
                    ...
                    device='/cpu:0',
                    scope=None):
    with tf.name_scope(scope, 'semi_ssd_losses'):
        labeled_losses = ssd_losses(labeled_preds['logits'],
                                    labeled_preds['localisations'],
                                    labeled_ground_truths['classes'],
                                    labeled_ground_truths['localisations'],
                                    labeled_ground_truths['scores'],
                                    match_threshold=match_threshold,
                                    negative_ratio=negative_ratio,
                                    alpha=alpha,
                                    label_smoothing=label_smoothing,
                                    device=device,
                                    scope='labeled')
        unlabeled_losses = ssd_losses(unlabeled_preds['logits'],
                                      unlabeled_preds['localisations'],
                                      unlabeled_pseudo_labels['classes'],
                                      unlabeled_pseudo_labels['localisations'],
                                      unlabeled_pseudo_labels['scores'],
                                      match_threshold=match_threshold,
                                      negative_ratio=negative_ratio,
                                      alpha=alpha,
                                      label_smoothing=label_smoothing,
